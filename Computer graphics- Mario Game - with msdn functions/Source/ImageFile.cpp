...
bool CImageFile::LoadBitmapFromFile(const char *szFileName, HDC hdc)
{
	...
	// Loads the image.
	m_hBMP = (HBITMAP)LoadImage(g_hInst, szFileName, IMAGE_BITMAP, GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN), LR_CREATEDIBSECTION | LR_LOADFROMFILE);
